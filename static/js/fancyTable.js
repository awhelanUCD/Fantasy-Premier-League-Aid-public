...
/*
Augmented by Andrew Whelan 2022
*/
      ...
      columnNames_: [],
			...
			sortFunction: function(a, b, fancyTableObject, rowA, rowB){
				...
				if(fancyTableObject.sortAs[fancyTableObject.sortColumn] == 'numeric'){
					return(
						(fancyTableObject.sortOrder>0) ? parseFloat(b)-parseFloat(a) : parseFloat(a)-parseFloat(b)
					);
				} else {
		...
		this.each(function() {
			...
			if(settings.searchable){
				...
				if(settings.globalSearch){
					...
					var th = $("<th>",{ style:"padding:2px; color: red;" }).attr("colspan",elm.fancyTable.nColumns);
            ...
            if (nInputElm<3){
						var th = $("<td>",{ style:"padding:2px;" , class:"col_"+settings.columnNames_[nInputElm],"data-n": nInputElm, });
						$(searchField).appendTo($(th));
						$(th).appendTo($(searchHeader));
          }
          else{
          //var th = $("<td>",{ style:"display:none", class:""+settings.columnNames_[nInputElm],"data-n": nInputElm, });
          //$(searchField).appendTo($(th));
          //$(th).appendTo($(searchHeader));
        }
						nInputElm++;
